...
export async function getTodaySummary(now: Date = new Date()) {
  ...
  const [dueToday, overdue, dueSoon, onLeave, pendingApproval] = await Promise.all([
    prisma.task.count({ where: { status: { notIn: ["DONE"] }, dueDate: { gte: today, lte: todayEnd } } }),
    prisma.task.count({ where: { status: { notIn: ["DONE"] }, dueDate: { lt: today } } }),
    prisma.task.count({ where: { status: { notIn: ["DONE"] }, dueDate: { gt: todayEnd, lte: soonEnd } } }),
